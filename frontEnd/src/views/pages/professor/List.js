        ...
        <div className="d-flex flex-wrap justify-content-between">
            <div className="d-flex flex-wrap">
                <div className="mb-2 mb-md-0 mr-1 add-new">
                    <Button.Ripple color="primary" onClick={() => history.push("/professor/adicionar")}>
...
const List = () => {
                ...
                <div className="d-flex flex-xl-row flex-column align-items-xl-center align-items-start py-xl-0 py-1">
                    <div className="user-img ml-xl-0 ml-2">
                        <img
                            ...
                            width="36"
                            src={noFoto}
                            alt={row.nome !== null ? row.nome : ''}
                        />
                    </div>
                    <div className="user-info text-truncate ml-xl-50 ml-0">
                        <span
                            title={row.nome !== null ? row.nome : ''}
                            className="d-block text-bold-500 text-truncate mb-0">
                            {row.nome !== null ? row.nome : ''}
                        </span>
                    </div>
                </div>
            )
            ...
            cell: row => (
                <div className="actions cursor-pointer">
                    <Edit
                        ...
                        size={20}
                        onClick={() => history.push("/professor/editar/" + row.matricula)}
                    />
                    <Trash2
                        size={20}
    ...
    const handleDeletar = (id, nome) => {
        let shouldDelete = window.confirm("Confirma a remoção do professor " + nome + "?");
        if (shouldDelete) {
            professorService.deletar(id)
                .then((response) => {
                    if (response.status === 200) {
                        setColor("success");
                        setMensagem("O professor " + nome + " foi removido com sucesso!");
                        setIsOpen(true);
                ...
                .catch((error) => {
                    setColor("danger");
                    setMensagem("Ocorreu um erro ao tentar remover o professor " + nome + ".");
                    setIsOpen(true);
                });
            setTimeout(() => {
                setIsOpen(false);
    ...
    return (
        <Row className="app-user-list">
            <Col sm="12">
                <Breadcrumbs
                    breadCrumbTitle="Lista de professores"
